...
class Scanner:
    ...
    def analyze_event(self, event: Event) -> list[Candidate]:
        """Deep-scan one user-selected event regardless of its start time."""
    ...
    def scan_live(self, now: datetime | None = None) -> list[Candidate]:
        """Low-frequency live scan. It only requests odds for sports with live events."""
        ...
        for sport in self.config.watchlist.sports:
            ...
            for event in live_events:
                try:
                    quotes = [
                        q for q in self.provider.quotes(event, self.config.live.markets)
                        if market_allowed(q, self.config.live.markets)
                    ]
                except Exception as exc:
                    print(f"Live scan error {event.home} vs {event.away}: {exc}")
                    continue
                for target in self.config.watchlist.bookmakers:
                    candidates.extend(self._evaluate_quotes(
                        event,
                        quotes,
                        target,
                        minimum_confidence=self.config.live.minimum_confidence,
                        minimum_edge=self.config.live.minimum_edge,
                        minimum_expected_value=self.config.live.minimum_expected_value,
                    ))
        ...
        for sport in self.config.watchlist.sports:
            ...
            for event in events:
                if not matches_watchlist(event, self.config.watchlist):
                    continue
                if alert_only:
                    # ALERT mode is now intentionally event-only. It must not burn odds credits.
                    continue
